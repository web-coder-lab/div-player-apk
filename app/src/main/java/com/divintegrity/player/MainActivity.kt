...
/**
 * Phase 2 — Approved popup + Go back + persistent notification + launcher hide
 */
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when {
            intent?.action == ScannerForegroundService.ACTION_OPEN_FROM_NOTIFICATION && joined -> {
                showApproved()
            }
    ...
    private fun startConnect() {
        step = Step.CONNECT
        hideInputs()
        badge.text = "PHASE 3 · CONNECT"
        title.text = "Div Scanner"
        subtitle.text = "Connecting to server…"
        progress.visibility = View.VISIBLE
        statusText.text = "Free server may take 30–60 seconds"
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (ok) {
                ...
                if (prefs.getBoolean("perms_done", false)) showUsername() else showPerms()
            } else {
                subtitle.text = "Server not reachable"
                statusText.text = last.ifBlank { "Check internet and retry" }
    ...
    private fun showPerms() {
        step = Step.PERMS
        badge.text = "PHASE 3 · PERMISSIONS"
        title.text = "Permissions"
        subtitle.text = "Allow notifications for scanner"
        statusText.text = "Needed so “Div scanner is running” stays visible"
    ...
    private fun showApproved() {
        ...
        hideInputs()
        runHardScan()
        badge.text = "PHASE 3 · APPROVED"
        title.text = "You are approved"
        subtitle.text = "Scanner notification is on"
    ...
    private fun showRunning() {
        ...
        hideInputs()
        runHardScan()
        badge.text = "PHASE 3 · RUNNING"
        title.text = "Div scanner is running"
        subtitle.text = "Notification stays on · tap it anytime"
        subtitle.setTextColor(0xFFA5B4FC.toInt())
        val sc = prefs.getInt("last_score", -1)
        val vd = prefs.getString("last_verdict", "") ?: ""
        statusText.text = buildString {
            append("Player: ${prefs.getString("username", "")}")
            if (sc >= 0) append("\nLast scan: $sc · $vd")
        }
        primaryBtn.visibility = View.VISIBLE
    ...
    private fun onPrimary() {
        ...
        when (step) {
            ...
            Step.APPROVED -> {
                // Go back — popup hides, notification remains
    ...
    private fun requestNotifPerm() {
        if (Build.VERSION.SDK_INT >= 33) {
            ...
            if (ContextCompat.checkSelfPermission(this, need) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(this, arrayOf(need), 1001)
            }
        }
    }
    ...
    private fun join(code: String) {
        ...
        prefs.edit().putString("last_code", code).apply()
        progress.visibility = View.VISIBLE
        primaryBtn.isEnabled = false
        statusText.text = "Joining via server…"
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (result == null) {
                showError("Server error — check internet & retry")
                return@launch
            }
    ...
    /** Phase 3 — passive signals → deep_v3_hard on server → owner results */
    private fun runHardScan() {
        val token = prefs.getString("token", "") ?: ""
        if (token.isBlank()) return
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val (signals, device) = SignalCollector.collect(this@MainActivity)
                val result = postJson(
                    "$deepBase/v1/player/scan/run",
                    mapOf("signals" to signals, "device" to device),
                    token
                )
                if (result?.get("ok")?.asBoolean == true) {
                    val data = result.getAsJsonObject("data")
                    val score = data?.get("score")?.asInt
                    val verdict = data?.get("verdict")?.asString
                    prefs.edit()
                        .putInt("last_score", score ?: -1)
                        .putString("last_verdict", verdict ?: "")
    ...
    private fun pingAuth(): String {
        return try {
            val req = Request.Builder().url("$authBase/health").get()
                .header("Accept", "application/json").build()
            client.newCall(req).execute().use { r ->
                val body = r.body?.string().orEmpty()
                if (r.isSuccessful && body.contains("\"ok\"")) "OK ${r.code}" else "FAIL HTTP ${r.code}"
            }
        } catch (e: Exception) {
    ...
    private fun postJson(url: String, body: Map<String, Any?>, token: String? = null): JsonObject? {
        return try {
            ...
            client.newCall(b.build()).execute().use { resp ->
                gson.fromJson(resp.body?.string().orEmpty(), JsonObject::class.java)
            }
        } catch (_: Exception) {
            null
        }
